 ### Get starting date/time ###
    ...
    $deployname="xeroxdemo2"
    $storageRGName="XeroxDemo2Store"
    $RGName="XeroxDemo2"
    $locName="Central US"
    $templateURI="https://raw.githubusercontent.com/eGistics/AzureTemplates/master/ConfigurationTemplates/XeroxDemo.json"

    ### Deleting Resource Group ###
    Remove-AzureRMResourceGroup -Name $RGName -Force

    ### Save the destination storage account key ###
    $destStorageKey = (Get-AzureRmStorageAccountKey -Name "xeroxdemo2ssd" -ResourceGroupName $storageRGName ).key1
    ...
    $destStorageAccount = "xeroxdemo2ssd"
    ...
    $params = @{"vmName"=$deployname;"osDiskVhdUri"="https://xeroxdemo2ssd.blob.core.windows.net/vhds/XeroxDemo.vhd";"osType"="Windows";"vmSize"="Standard_F4s";}
    $deploymentOutput = (New-AzureRMResourceGroupDeployment -Name $deployName -ResourceGroupName $RGName -TemplateUri $templateURI -TemplateParameterObject $params).ProvisioningState

    ### Get Deployment Completion Date ###
    $EndDate = get-date
    
    #Send Deployment Results
    $subject = "XeroxDemo2 Deployment Results"
